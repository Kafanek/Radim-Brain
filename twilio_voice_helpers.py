...
def generate_azure_tts(text, rate_pct=None, pitch_hz=None, mode="HARMONY", user_id=None):
    ...
    try:
        ...
        # Sprint AA: also fetch full brain_speech dict so phone calls get
        # Ψ(t)-driven rate/pitch/pause from compute_unified_speech.
        _rtcf_voice = None
        _brain_speech = None
        if user_id:
            try:
                from brain_speech import get_brain_speech_for_user
                _brain_speech = get_brain_speech_for_user(user_id)
                if _brain_speech:
                    _rtcf_voice = _brain_speech.get('rtcf_voice')
            except Exception:
                pass
        ...
        try:
            ...
            ssml = build_radim_ssml(text, mode=mode, voice=RADIM_AZURE_VOICE, user_id=user_id,
                                    rtcf_voice=_rtcf_voice, brain_speech=_brain_speech)
        except ImportError:

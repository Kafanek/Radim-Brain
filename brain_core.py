...
def compute_psi_state(C, alpha, voice_tone=0.5, hrv=0.5, speech_tempo=0.5, user_id=None):
    ...
    # Sprint AE: print() (not logger.warning) for failures because gunicorn
    # swallows the logger output here. Bare print(flush=True) reaches the
    # Heroku log stream so silent fails surface immediately.
    try:
